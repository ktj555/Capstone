...
real q_boil(cell_t c,Thread* t){
    ...
    return alpha_sf(c,t) * Viscosity_m(c,t) * H_fg(c,t) * sqrt(9.81 * (Rho_l(c,t) - Rho_v(c,t)) / figure.sigma) * pow(Specific_Heat_l(c,t) * (T_s - T_sat(c,t)) / (figure.c_sf * H_fg(c,t) * Pr_l(c,t)), 3);
}
...
real dqboil_dS(cell_t c,Thread* t){
    ...
    return alpha_sf(c,t) * dnu_dS(c,t) * H_fg(c,t) * sqrt(9.81 * (Rho_l(c,t) - Rho_v(c,t)) / figure.sigma) * pow(Specific_Heat_l(c,t) * (T_s - T_sat(c,t)) / (figure.c_sf * H_fg(c,t) * Pr_l(c,t)) , 3);
}
...
real inlet_enthalpy_l(face_t f,Thread* t){
	...
	m_flux = mass_in / (pow(figure.D,2) / 4);
    ...
    return (figure.reservoir_enthalpy + h_l(f,t) / m_flux * T_s - Conductivity_m(f,t) / m_flux / Specific_Heat_l(f,t) * dhdx) / (1 + h_l(f,t) / m_flux / Specific_Heat_l(f,t));
}
real inlet_enthalpy_v(face_t f,Thread* t){
    ...
    return (figure.reservoir_enthalpy + h_v(f,t) / m_flux * (T_s - T_sat(f,t) + H_sat_v(f,t) / Specific_Heat_v(f,t)) - Conductivity_m(f,t) * Porosity(f,t) / Specific_Heat_v(f,t) * dhdx) / (1+h_v(f,t) / m_flux / Specific_Heat_v(f,t));
}
real inlet_enthalpy_m(face_t f,Thread* t){
    ...
    h_inlet = S_(f,t) * h_l(f,t) + (1-S_(f,t)) * h_v(f,t);
    ...
    return (figure.reservoir_enthalpy + h_inlet / m_flux * (T_s - T_sat(f,t)) - Conductivity_m(f,t) * Porosity(f,t) / m_flux * dTsat_dP_face(f,t) * dPdx) / beta_current(f,t);
}
